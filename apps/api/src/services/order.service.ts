...
export class OrderService {
  ...
  async getAllOrders(
    page: number = 1,
    pageSize: number = 10,
    sortFieldAdmin: "created_at" = "created_at",
    ...
    try {
      ...
      if (orderStatus.length > 0) {
        whereCondition.order_status = { in: orderStatus };
      }
      if (storeName) {
  ...
  async getStoreOrders(
    ...
    try {
      ...
      if (orders.length === 0)
        return { message: "No orders found for this store." };
      const mappedOrders = orders.map((order) => ({
        order_id: order.order_id,
        username: order.User.username,
        address: order.Address.address,
...
async changeOrderAddress(
  ...
  try {
    ...
    const { isValidAddress, message } =
      await this.profileService.validateAddressForStore(
        order.store_id,
        address_id
    );
  ...
  async processOrder(store_id: number, order_id: number) {
    try {
      const order = await this.prisma.orders.findFirst({
        ...
        include: {
          Cart: { include: { CartItems: { include: { Inventory: true } } } },
        },
      });
      if (!order) {
      ...
      if (order.order_status !== "PROCESSING") {
        return {
          error: "Order cannot be processed as it is not in PROCESSING state.",
        };
      }
      ...
      for (const cartItem of cart.CartItems) {
        ...
        if (!inventory) {
          return {
            error: `Inventory not found for id: ${cartItem.inventory_id}`,
          };
        }
        if (
          inventory.user_stock === null ||
          inventory.user_stock < cartItem.quantity
        ) {
  ...
  async sentOrder(order_id: number) {
    try {
      const updatedOrder = await this.prisma.orders.update({
        where: { order_id },
        data: { order_status: "SENT" },
      });
      setTimeout(async () => {
        await this.prisma.orders.update({
          where: { order_id },
          data: { order_status: "ORDER_CONFIRMED" },
        });
      }, 604800000);
  ...
  async cancelUserOrder(user_id: number, order_id: number) {
    ...
    if (order.order_status !== "PENDING_PAYMENT") {
      return {
        error:
          "Order cannot be cancelled as it is not in PENDING_PAYMENT state.",
      };
    }
    const updatedOrder = await this.prisma.orders.update({
      where: { order_id },

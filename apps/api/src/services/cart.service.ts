...
export class CartService {
    ...
    try {
      let cart =
        ...
        (await this.prisma.carts.create({
          data: { user_id, cart_price: 0, is_active: true },
        }));
      ...
      const existingCartItem = await this.prisma.cartItems.findFirst({
        where: { cart_id: cart.cart_id, inventory_id },
      });
      if (existingCartItem)
      ...
      const newCartItem = await this.prisma.cartItems.create({
        data: {
          ...
          inventory_id,
          quantity: 1,
          product_price: inventory.discounted_price || 0,
        },
      });
  ...
  async updateCartPrice(user_id: number) {
    try {
      ...
      const {
        _sum: { product_price },
      } = await this.prisma.cartItems.aggregate({
        where: { cart_id: cart?.cart_id },
        _sum: { product_price: true },
      });
      ...
      const updatedCartPrice = await this.prisma.carts.update({
        where: { cart_id: cart?.cart_id },
        data: { cart_price: newCartPrice },
      });
  ...
  async changeItemQuantity(
    ...
    try {
      ...
      if (quantity > 1) {
        finalPrice = await checkAndApplyBogo(
          cartItem.inventory_id,
          updatedPrice
        );
      }
      const updatedCartItem = await this.prisma.cartItems.update({
        where: { cart_item_id },
        data: { quantity, product_price: finalPrice },
      });
  ...
  async getCartItems(user_id: number) {
    try {
      const cart = await this.prisma.carts.findFirst({
        where: { user_id, is_active: true },
        include: {
          CartItems: { include: { Inventory: { include: { Product: true } } } },
        },
      });
      if (!cart) {
        return { cart_price: 0, cart_items: [] };
      }
      return {
        cart_id: cart.cart_id,
        ...
        cart_items: cart.CartItems.map((item) => ({
          ...
          original_price: item.Inventory.Product.price,
  ...
  async removeCartItem(user_id: number, cart_item_id: number) {
    try {
      ...
      if (!cartItem) return { error: "Cart item not found" };
      const deletedItem = await this.prisma.cartItems.delete({
        where: { cart_item_id },
      });
      return deletedItem;
    } catch (error) {
  ...
  async checkoutCart(user_id: number) {
    try {
      ...
      if (!cart || cart.CartItems.length === 0)
        return { error: "Active cart not found or empty" };
      const address = await this.prisma.address.findFirst({
        where: { user_id, is_default: true },
      });
      if (!address) return { error: "No default address found" };
      const store = await this.prisma.stores.findUnique({
        where: { store_id: cart.CartItems[0].Inventory.store_id },
      });
      if (!store) return { error: "Store not found" };
      const storeDiscount = await wholeStoreCartDiscount(
        Number(cart.cart_price),
        store.store_id
      );
      ...
      const voucherResponse = await this.voucherService.sendCartVoucher(
        user_id,
        Number(cart.cart_price)
      );
      return {
  ...
  async calculateShippingPrice(order_id: number) {
    try {
      const order = await this.prisma.orders.findUnique({
        where: { order_id },
        include: { Cart: { include: { CartItems: true } }, Address: true },
      });
      if (!order) throw new Error("Order not found");
      const totalWeight = order.Cart.CartItems.reduce(
      ...
      const payload = {
        ...
        destination: order.Address.city_id,

...
import { sendErrorResponse, sendZodErrorResponse } from "../utils/response.utils";
...
export class ProfileController {
  ...
  async addAddress(req: Request, res: Response): Promise<void> {
    ...
    const data: any = await this.profileService.addAdress(
      user_id,
      address,
      city_name,
      city_id
  ...
  async changeDefaultAddress(req: Request, res: Response): Promise<void> {
    ...
    const data: any = await this.profileService.changeDefaultAddress(
      user_id,
      new_address_id
    );
    if (data && !data.error) {
  ...
  async updateUserProfile(req: Request, res: Response) {
    try {
      ...
      const updateUserProfile = await this.profileService.updateUserProfile(
        user_id,
        { username, email }
      );
      res.status(201).send({
